...
void GL_AddFlareSurface (msurface_t *surf )
{
	...
	if (surf->texinfo->flags & 
	    (SURF_SKY|SURF_TRANS33|SURF_TRANS66|SURF_FLOWING|SURF_DRAWTURB|SURF_WARP))
		return;
	...
	height = surf->texinfo->image->upload_height;
        ...
        buffer = Q_malloc(width * height * 3);
        ...
        VectorScale(rgbSum, gl_lensflare_intens->value / (width *height), color);
...
void Mod_LoadLeafs (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		if (out->contents & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA/*|CONTENTS_THINWATER*/) ) 
		{
			...
			{
				if ((out->firstmarksurface[j]->texinfo->flags  &
...
void R_BeginRegistration (char *model)
{
	...
	if (gl_lightmap_texture_saturation->value > 1 || gl_lightmap_texture_saturation->value < 0)
		ri.Cvar_Set("gl_lightmap_texture_saturation", "1");
