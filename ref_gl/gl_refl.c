...
setupShaders(void)
{
	...
	if (len == -1) {
		...
		ri.Con_Printf(PRINT_ALL, "Water fragment program (scripts/water1.arbf) not found, disabling it.\n");
		gl_state.fragment_program = false;
		return;
	}
	fragmentProgramText2 = (char *)Q_malloc(len + 1);
	...
	qglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, 
	                    len, fragmentProgramText2);

	brightenTexture = false; /* dont brighten these textures we need them as they are.  */

	distortTex = Draw_FindPic(gl_reflection_shader_image->string);
	waterNormalTex = Draw_FindPic("/textures/water/normal.pcx");

	brightenTexture = true;	/* reset so normal textures load with extra brightness  */

	ri.FS_FreeFile(fragmentProgramText);	/* clear up  */
	Q_free(fragmentProgramText2);	/* clear up  */

	if (!waterNormalTex || !distortTex) {
		gl_state.fragment_program = false;
		ri.Con_Printf(PRINT_ALL,"Water distortion texture not found, disabling shader reflection.\n");
	}
}
...
R_shutdown_refl(void)
{
	...
	if (gl_state.fragment_program || gl_reflection_fragment_program->value) { //FIXME
		qglDeleteProgramsARB(1, &gWaterProgramId);	 /* free fragment program */
	}
	R_clear_refl();		/* set number of reflections to 0 */

	Q_free(g_refl_X);		/* free all other malloc'd stuff */
	Q_free(g_refl_Y);
	Q_free(g_refl_Z);
	Q_free(g_tex_num);
	Q_free(g_waterDistance);
	Q_free(g_waterDistance2);
	Q_free(waterNormals);
	/* Q_free( playerEntity		);	//fix this later */
}
...
R_init_refl(int maxNoReflections)
{
	 ...
	 * will be left again so maybe its best to leave this alone.
	 */
	...
	for (i = 0; i < maxReflections; i++) {
		...
		buf = (unsigned char *)Q_malloc(REFL_TEXW * REFL_TEXH * 3); /* create empty buffer for texture */
		...
		if (buf) {
			...
			Q_free(buf);	/* once we've made texture memory, we
	...
	ri.Con_Printf(PRINT_DEVELOPER, "...reflective texture size set at %d\n", g_reflTexH);
	ri.Con_Printf(PRINT_DEVELOPER, "...maximum reflective textures %d\n\n", maxReflections);
...
R_setupArrays(int maxNoReflections)
{
	...
	g_refl_X = (float *)Q_malloc(sizeof(float) * maxNoReflections);
	g_refl_Y = (float *)Q_malloc(sizeof(float) * maxNoReflections);
	g_refl_Z = (float *)Q_malloc(sizeof(float) * maxNoReflections);
	g_waterDistance = (float *)Q_malloc(sizeof(float) * maxNoReflections);
	g_waterDistance2 = (float *)Q_malloc(sizeof(float) * maxNoReflections);
	g_tex_num = (int *)Q_malloc(sizeof(int) * maxNoReflections);
	waterNormals = (vec3_t *) Q_malloc(sizeof(vec3_t) * maxNoReflections);
...
R_DoReflTransform(qboolean update)
{
	...
	qglRotatef(-r_newrefdef.viewangles[2], 1, 0, 0);	/* MPO : this handles rolling 
...
drawPlayerReflection(void)
{
	...
	if (playerEntity == NULL) {
		...
		playerEntity = (entity_t *) Q_malloc(sizeof(entity_t));
	...
	playerEntity->origin[0] = r_newrefdef.vieworg[0];
	playerEntity->origin[1] = r_newrefdef.vieworg[1];
	playerEntity->origin[2] = r_newrefdef.vieworg[2];

	playerEntity->oldorigin[0] = r_newrefdef.vieworg[0];
	playerEntity->oldorigin[1] = r_newrefdef.vieworg[1];
	playerEntity->oldorigin[2] = r_newrefdef.vieworg[2];

	playerEntity->frame = 30;
	playerEntity->oldframe = 30;

	playerEntity->angles[0] = r_newrefdef.viewangles[0];
	playerEntity->angles[1] = r_newrefdef.viewangles[1];
	playerEntity->angles[2] = r_newrefdef.viewangles[2];
