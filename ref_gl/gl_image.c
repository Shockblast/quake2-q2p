...
void GL_Bind_2 (int texnum, image_t *tex)
{
...
void GL_TextureMode( char *string )
{
	...
	ri.Con_Printf (PRINT_ALL, "Texture mode: %s\n", modes[i].name);
...
void LoadTGA( char *filename, byte **pic, int *width, int *height )
{
	...
	if( mapped ) {
		...
		ColorMap = (byte *)Q_malloc( MAXCOLORS * 4 );
		map_idx = 0;
...
void LoadJPG(char *filename, byte ** pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	rgbadata = Q_malloc(cinfo.output_width * cinfo.output_height * 4);
	if (!rgbadata) {
	...
	*height = cinfo.output_height;
	...
	scanline = Q_malloc(cinfo.output_width * 3);
	if (!scanline) {
...
void InitializeDemData(void)
{
	...
	if (my_png->FRowPtrs) {
		Q_free(my_png->FRowPtrs);
		my_png->FRowPtrs = 0;
	}
	my_png->Data = Q_malloc(my_png->Height * my_png->FRowBytes);	/* DL Added 30/5/2000 */
	my_png->FRowPtrs = Q_malloc(sizeof(void *) * my_png->Height);
...
void mypng_struct_create(void)
{
	if (my_png)
		return;
	my_png = Q_malloc(sizeof(png_t));
	my_png->Data = 0;
	my_png->FRowPtrs = 0;
	my_png->Height = 0;
	my_png->Width = 0;
...
void desaturate_texture(unsigned *udata, int width, int height)
{
	// jitsaturation
	int		i, size;
	float		r, g, b, v, s;
...
qboolean GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap)
{
	...
	if (gl_round_down->value && scaled_width > width && mipmap) scaled_width >>= 1;
	if (gl_round_down->value && scaled_height > height && mipmap) scaled_height >>= 1;

	// let people sample down the world textures for speed
	if (gl_picmip->value > 4.0f) gl_picmip->value = 4.0f; // jit
	...
	// jitsaturation
	if (gl_lightmap_texture_saturation->value < 1) desaturate_texture(scaled, scaled_width, scaled_height);
	...
	if (mipmap) {
		...
		if (gl_state.sgis_mipmap) {
			...
			while (scaled_width > 1 || scaled_height > 1) {
				...
				miplevel++;
				qglTexImage2D(GL_TEXTURE_2D, miplevel, comp, scaled_width, scaled_height, 0, 
				              GL_RGBA, GL_UNSIGNED_BYTE, scaled);
			}
		}
		...
		// jitanisotropy
		if (gl_anisotropy->value) qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, gl_anisotropy->value);
		else qglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, gl_filter_max);
	}
	else {
		if (brightenTexture)
			GL_LightScaleTexture(scaled, scaled_width, scaled_height, !mipmap);
...
image_t *GL_LoadPic (char *name, byte *pic, int width, int height, imagetype_t type, int bits)
{
	...
	if( !strcmp(s + len - 4, ".png") || !strcmp(s + len - 4, ".tga") || !strcmp(s+len-4, ".jpg"))
	{
		...
		if (mt) {
			...
			if (pcxwidth > 0 && pcxheight > 0) {
				image->replace_scale = ((float)pcxwidth / image->width + (float)pcxheight / image->height)
				                         * 0.5;
...
image_t	*GL_FindImage (char *name, imagetype_t type)
{
	...
	//Knightmare- MrG's automatic JPG & TGA loading
	...
	if (!strcmp(name+len-4, ".pcx") || !strcmp(name+len-4, ".wal"))
	{
		...
		//Knightmare- if no TGA, check for JPG
...
void GL_FreeUnusedImages (void)
{
	...
	for (i=0; i<PARTICLE_TYPES; i++)
		if (r_particletexture[i]) //dont mess with null ones silly :p
