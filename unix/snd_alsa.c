...
SNDDMA_Init(struct sndinfo *s)
{
	...
	if (!si->dma->speed) {
		for (i=0 ; i<sizeof(tryrates); i++) {
			...
			err = snd_pcm_hw_params_set_rate_near(playback_handle, hw_params, 
				...
				else if (si->s_khz->value == 11) si->dma->speed = 11025;
				else  si->dma->speed = test;
				break;
      			}
    		}
  	}
	...
	if (si->dma->channels < 1 || si->dma->channels > 2)
		si->dma->channels = 2;
		err = snd_pcm_hw_params_set_channels(playback_handle,hw_params,
		                                     si->dma->channels);
		if (err < 0) {
	...
	si->dma->buffer = Q_malloc(buffer_bytes);  /* allocate pcm frame buffer */
