...
// perform a variable print or set
	if (Cmd_Argc() == 1) {
		if ((v->flags & CVAR_LATCH) && v->latched_string)
			Com_Printf("\"%s\" is \"^1%s^r\" : default is \"^3%s^r\" : latched to \"%s\"\n", v->name, v->string, v->default_string, v->latched_string);
		else
			Com_Printf("\"%s\" is \"^1%s^r\" : default is \"^3%s^r\"\n", v->name, v->string, v->default_string);
		/* Com_Printf ("\"%s\" is \"%s\"\n", v->name, v->string); */
		return true;
	}
...
void Cvar_List_f (void)
{
	...
	Com_Printf("\n^3%i^r variables\n", i );
	Com_Printf("^1'A'^r = Archive\n");
	Com_Printf("^2'U'^r = Userinfo\n");
	Com_Printf("^3'S'^r = Serverinfo\n");
	Com_Printf("^5'L'^r = Latch\n");
	Com_Printf("^9'-'^r = No set\n\n");
}
