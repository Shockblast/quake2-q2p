...
#ifdef _WIN32
#include "../win32/include/vorbisfile.h"
#else
...
void OGG_Init(void)
{
	...
	ogg_check = Cvar_Get("ogg_check", "0", 0);
...
void OGG_LoadFileList(void)
{
	...
	ogg_filelist = Q_malloc(sizeof(char *) * ogg_numfiles);
...
qboolean OGG_Open(ogg_seek_t type, int offset)
{
	...
	Com_Printf("Playing file '^3%s^r' [%d]\n", ogg_filelist[pos], pos);
