...
void CompleteCommand (void)
{
	...
	if (cmd)
	{
		...
		key_linepos = (unsigned int)strlen(cmd)+2;
...
void Key_Message (int key)
{
	...
	if ((toupper(key) == 'V' && keydown[K_CTRL]) ||
		...
		if ((cbd = Sys_GetClipboardData()) != 0) {
			...
			i = strlen(cbd);
			if (i + chat_bufferlen >= MAXCMDLINE)
				i = MAXCMDLINE - chat_bufferlen;

			if (i > 0) {
				cbd[i] = 0;
				strcat (chat_buffer, cbd);
				chat_bufferlen += i;
			}
			Q_free(cbd);
		}
...
void Key_Event (int key, qboolean down, unsigned key_time)
{
	...
	if (key == K_ENTER) {
		if (down && keydown[K_ALT])
		{
			...
			{
				Com_Printf("Switching to fullscreen mode\n");
				Cvar_Set("vid_fullscreen", "1");
			}
			else
			{
				Com_Printf("Switching to windowed mode\n");
