...
** VID_CreateWindow
*/
#define	WINDOW_CLASS_NAME	Q2PVERSION
...
qboolean GLimp_InitGL (void)
{
	...
	*/
	if ( strstr( gl_driver->string, GL_DRIVER_LIB ) != 0 )
	...
	*/
	...
	gl_state.hwgamma=GetDeviceGammaRamp(glw_state.hDC,original_ramp);
	if (gl_state.hwgamma)
...
void UpdateHardwareGamma(void)
{
	...
	memcpy(gamma_ramp, original_ramp, sizeof(gamma_ramp));
